...
description: |
  ...
  "AI slop". Use when asked to make writing not sound like AI, make docs clear
  or plain, enforce a controlled writing style, or write technical documentation
  that reads human. Two modes — strict (procedures/safety) and STE-flavored
  (general prose). Triggers on "ste", "STE", "simplified technical english",
  "de-slop", "anti-slop", "make this not sound like AI", "clean up this doc".
  Complements humanizer: STE prevents slop during generation, humanizer removes
  it after the fact.
...
for marketing copy, essays, or anything that needs a voice — STE strips voice
on purpose.
...
- Use the short common word: start (not begin/commence/initiate), use (not
  ...
  (not prior to), after (not subsequent to), about (not regarding/concerning),
  get (not obtain/acquire), show (not demonstrate), also (not
...
PUNCTUATION
- No semicolons. Write two sentences. (Note: the em dash is not banned by STE,
  only the semicolon is — add "no em dash" yourself if you want it gone.)
...
- **STE-flavored** — general prose (READMEs, PR descriptions, docs): apply the
  ...
  ~900-word dictionary lockdown so the text keeps enough range to read naturally.
...
After writing, optionally lint the output with the companion script to get a
quantitative score:

```
python3 ~/dotfiles/scripts/ste_lint.py <file>
```

Score is violations per 100 words — lower is cleaner. The delta between the
before and after scores IS the signal. A score under 2.0/100w is clean;
under 1.0/100w is exceptional.
