...
description: >
  ...
  scythe pipeline. Triggers on "generate a video", "make a promo video",
  "create a marketing video", "brutalist video", "kinetic video", "scythe",
  "scythe video", or any request to produce video content from project assets.
allowed-tools: read, write, edit, bash
---

# scythe - Kinetic Video Generation

Generate brutalist, beat-synced short-form videos from a project's assets using
the scythe pipeline. This skill orchestrates: bootstrap → asset ingestion →
LLM-generated cutlist → containerized render.

## Engine Location

The scythe engine lives at `~/1-projects/scythe/`. It is an independent
...
| Script | Purpose |
|--------|---------|
| `~/dotfiles/scripts/scythe_build.sh` | Ensure the Podman container image exists |
| `~/dotfiles/scripts/scythe_bootstrap.sh <dir>` | Scaffold a video project directory |
| `~/dotfiles/scripts/scythe_ingest.sh <dir> [opts]` | Fetch royalty-free audio, video, images |
| `~/dotfiles/scripts/scythe_render.sh <dir> [opts]` | Build container + render MP4 |

The prompt template is at `~/.pi/agent/prompts/scythe-cutlist.md`.
...
2. **Inside current project:** If the user is working in a project (CWD), create
   `<cwd>/scythe-video/` as the video project directory
...
```bash
bash ~/dotfiles/scripts/scythe_bootstrap.sh <absolute-target-dir>
```
...
```bash
bash ~/dotfiles/scripts/scythe_ingest.sh <dir> --audio-query "<vibe>"
```
...
```bash
bash ~/dotfiles/scripts/scythe_ingest.sh <dir> --no-audio --ia-id <identifier>
```
...
1. **Read the prompt template:**
   ```
   ~/.pi/agent/prompts/scythe-cutlist.md
   ```
...
```bash
bash ~/dotfiles/scripts/scythe_render.sh <project-dir>
```
...
- Run `~/1-projects/scythe/scripts/install_deps.sh` if deps are missing

## Verification

- `~/dotfiles/scripts/scythe_build.sh` must succeed before render
